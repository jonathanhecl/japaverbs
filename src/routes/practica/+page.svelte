<script lang="ts">
	...
	type GameMode = 'menu' | 'flashcards' | 'multiple-choice' | 'conjugation' | 'listening';
	...
	const games = [
		...
		},
		{
			id: 'conjugation',
			title: 'Conjugación',
			description: 'Practica las formas verbales',
			icon: '📝',
	...
	function startGame(mode: GameMode) {
		currentMode = mode;
		gameVerbs = shuffleArray([...verbs]).slice(0, 10);
		currentIndex = 0;
		questionCount = 0;
		correctCount = 0;
		loadNextQuestion();
			...
			<div class="grid gap-4">
				{#each games as game}
					<button
						...
						class="group relative overflow-hidden rounded-2xl border border-slate-800 bg-gradient-to-br {game.color} text-left p-[1px] transition-all hover:-translate-y-1 active:translate-y-0"
					>
						...
						<div class="absolute inset-0 backface-hidden rotate-y-180 rounded-3xl border-2 border-green-500 bg-gradient-to-br from-green-600/20 to-emerald-600/20 p-8 flex flex-col items-center justify-center text-center">
							...
							{#if currentVerb.examples.length > 0}
								<div class="space-y-3 w-full">
									<div class="text-sm text-slate-300 bg-slate-900/50 rounded-xl p-4 border border-slate-800">
										<p class="font-japanese mb-2">{currentVerb.examples[0].ja}</p>
										<p class="text-slate-400">{currentVerb.examples[0].es}</p>
									</div>
								</div>
							{/if}
